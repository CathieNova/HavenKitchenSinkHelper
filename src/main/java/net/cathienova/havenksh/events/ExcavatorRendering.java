...
public class ExcavatorRendering {
    ...
    public static void renderBlockHighlight(RenderHighlightEvent.Block event) {
        ...
        if (!(itemStack.getItem() instanceof ExcavatorBase)) return;
        ...
        double dy = player.yOld + (player.getY() - player.yOld) * partialTicks + player.getEyeHeight();
        ...
        for (BlockPos blockPos : blocks) {
            ...
            LevelRenderer.renderLineBox(poseStack, vertexConsumer, aabb,
                    (float)HavenConfig.excavator_highlight_red,
                    (float)HavenConfig.excavator_highlight_green,
                    (float)HavenConfig.excavator_highlight_blue,
                    (float)HavenConfig.excavator_highlight_alpha);
        }
        ...
        if (direction == Direction.DOWN || direction == Direction.UP) {
            for (int x = -range; x <= range; x++) {
                for (int z = -range; z <= range; z++) {
                    ...
                    if (player.level().getBlockState(pos).is(BlockTags.MINEABLE_WITH_SHOVEL)) {
                        positions.add(pos);
                    }
                }
            }
